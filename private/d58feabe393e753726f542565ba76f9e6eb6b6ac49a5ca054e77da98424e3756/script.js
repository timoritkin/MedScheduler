const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbzzPLAJNucXJBYWchyKAK_-PvNEhx6lTU5y3v94Vq_gSS2VVjN8cjCWrUFpecr1eEpEZQ/exec";
...
function patientSentKey(name, phone, date) {
  return `${name}|${phone}|${date}`;
}

function getWaSentState() {
  try {
    return JSON.parse(localStorage.getItem(WA_SENT_KEY) || '{}');
  } catch {
    return {};
  }
}

function clearWaSent(name, phone, date) {
  const key = patientSentKey(name, phone, date);
  const sent = getWaSentState();
  if (!sent[key]) return;
  delete sent[key];
  localStorage.setItem(WA_SENT_KEY, JSON.stringify(sent));
}

function markWaSent(btn, type, name, phone, date) {
  const key = patientSentKey(name, phone, date);
  const sent = getWaSentState();
  ...
  btn.innerHTML = WA_SVG + (type === 'msg1' ? ' ✅ הודעה ראשונה — נשלח' : ' ✅ תזכורת — נשלח');
}
...
function displayCards(patients) {
  ...
  sortedDates.forEach(date => {
    ...
    groups[date].forEach(p => {
      ...
      const sentKey = patientSentKey(p.name, p.phone, p.date);
      const sent = waSent[sentKey] || {};
      const msg1Sent = sent.msg1 ? ' sent' : '';
      const msg2Sent = sent.msg2 ? ' sent' : '';
      const msg1Label = sent.msg1 ? ' ✅ הודעה ראשונה — נשלח' : ' הודעה ראשונה';
      const msg2Label = sent.msg2 ? ' ✅ תזכורת — נשלח' : ' תזכורת';
      html += '<div class="patient-card" style="animation-delay:' + (idx*0.04) + 's" id="' + cardId + '">' +
        ...
        '<div class="card-actions">' +
          '<a href="' + waLink1 + '" target="_blank" class="wa-btn wa-btn-1' + msg1Sent + '" onclick="markWaSent(this, \'msg1\', \'' + esc(p.name) + '\', \'' + esc(p.phone) + '\', \'' + esc(p.date) + '\')">' + WA_SVG + msg1Label + '</a>' +
          '<a href="' + waLink2 + '" target="_blank" class="wa-btn wa-btn-2' + msg2Sent + '" onclick="markWaSent(this, \'msg2\', \'' + esc(p.name) + '\', \'' + esc(p.phone) + '\', \'' + esc(p.date) + '\')">' + WA_SVG + msg2Label + '</a>' +
        '</div>' +
      '</div>';
      idx++;
    });
...
async function changePatientDateTime(cardId, name, surname, phone, currentDate, currentTime) {
  ...
  clearWaSent(name, phone, currentDate);
